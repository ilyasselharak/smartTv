...
<div className={` ${(path==='/checkout'?'':'fixed  ')} duration-500   top-0 shadow-shad left-0 z-[1] w-full`}>
    ...
    <Link href="/" >
     ...
     <ul id="navbar" className={`md:flex bg-white text-red-600 md:items-center md:pb-0 pb-12 absolute md:static md:z-auto z-[5] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in ${open ? 'top-20':'top-[-412px]'}`}>
        <li className={`md:ml-8 md:my-0 my-7 font-bold ${open ? 'hover:border-b hover:border-red-300 hover:text-xl':''}`}><Link href={'/'} className={`${(path==='/'?'text-black text-xl border-red-300 border-b':'hover:text-black ')} duration-500 hover:text-red-600  `}>HOME</Link></li>
        ...
        <li className={`md:ml-8 md:my-0 font-bold text-red-400 my-7 `}><Link onClick={()=>setOpen(!open)} href={'/buisness'} className={`${(path==='/buisness'?'text-black text-xl border-red-300 border-b':'')} duration-500   `}>BUSINESS</Link></li>
